...
const ServiceLaborNewsViewDetail = () => {
  const { id } = useParams();
  ...
  const getOne = async () => {
    setLoading(true);
    try {
      const res = await request("servicelabornews/getone", "post", { id });
      if (res && res.data) {
        setBlog(res.data);
  ...
  const fetchImages = async (trainingId) => {
    const res_image = await request("tbservicelabornews_image/" + trainingId, "get");
...
export default ServiceLaborNewsViewDetail;
