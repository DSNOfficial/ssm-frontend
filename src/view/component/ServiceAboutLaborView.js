...
const ServiceAboutLaborView = () => {
  ...
  const getOne = async () => {
    setLoading(true);
    try {
      const res = await request(`about-labor/getone`, "post", { id });
      if (res && res.data) {
        setBlog(res.data);
        setTimeout(() => {
  ...
  return (
    <div style={containerStyle} ref={contentRef}>
      <Spin spinning={loading}>
         <div style={{ fontFamily: "'Khmer OS Siemreap', 'Noto Sans Khmer', sans-serif", color: "#0B22C2" }}>
...
export default ServiceAboutLaborView;
