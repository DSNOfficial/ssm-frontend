...
const ServiceLaborNewsView = () => {
  ...
  const getList = async () => {
    setLoading(true);
    try {
      const res = await request("servicelabornews/getList", "get");
          ...
          {paginatedList.map((item, index) => (
            <Col key={index} xs={24} sm={12} md={12} lg={6}>
              <Card
                ...
                style={{ width: '100%' }}
                cover={
                  <NavLink to={`/page/servicelabornews/${item.id}`}>
...
export default ServiceLaborNewsView;
