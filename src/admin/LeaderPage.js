...
const LeaderPage = () => {
    ...
    const getList = async () => {
        ...
        try {
            const res = await request("leader/getList", "get", param);
            if (res) {
                setList(res.list);
            }
        } catch (error) {
    ...
    const onClickBtnDelete = (item) => {
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                try {
                    const res = await request("leader/delete", "delete", data);
                    if (res) {
    ...
    const onFinish = async (item) => {
        ...
        const url = (id == null ? "leader/create" : "leader/update");
        try {
    ...
    return (
        <MainPage loading={loading}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 10 }}>
                <Space>
                    <div className="txt_title">ថ្នាក់ដឹកនាំមន្ទីរពេទ្យ</div>
            ...
            <Modal
                title={isEditMode ? "កែប្រែ | ថ្នាក់ដឹកនាំមន្ទីរពេទ្យ" : "បន្ថែម | ថ្នាក់ដឹកនាំមន្ទីរពេទ្យ"}
                open={open}
                maskClosable={false}
