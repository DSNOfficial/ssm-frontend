...
const ServicePackagePage = () => {
    ...
    const getList = async () => {
        ...
        const res = await request("servicecontact/getList", "get", param);
        setLoading(false);
        if (res) {
            setList(res.list);
        }
    ...
    const onClickBtnDelete = async (item) => {
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                const res = await request("servicecontact/delete", "delete", data);
                if (res) {
    ...
    const onFinish = async (item) => {
        ...
        form.append("title", item.title);
         form.append("etitle", item.etitle);
        ...
        var url = "servicecontact/create"
        if (formCat.getFieldValue("id")) {
            url = "servicecontact/update"
        }
    ...
    return (
        <MainPage loading={loading}>
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 10 }}>
                <Space>
                    <div className="txt_title">កញ្ចប់សេវា
</div>
                        ...
                        <Col span={24}>
                            <Form.Item
                                ...
                                rules={[
                                    {
                                        required: true,
                                        message: 'សូមបំពេញតម្លៃ',
                                    },
                                ]}
                            >
                                <Input style={fontSize} placeholder="តម្លៃ" />
                            </Form.Item>
                        </Col>
                    </Row>
                    <Row>
