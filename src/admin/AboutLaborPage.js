...
const AboutLaborPage = () => {
    ...
    const getList = async () => {
        ...
        try {
            const res = await request("about-labor/getList", "get", param);
            if (res) {
                setList(res.list);
            }
        } catch (error) {
    ...
    const onClickBtnDelete = (item) => {
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                try {
                    const res = await request("about-labor/delete", "delete", data);
                    if (res) {
    ...
    const onFinish = async (item) => {
        ...
        var url = "about-labor/create";
        if (formCat.getFieldValue("id")) {
            url = "about-labor/update";
        }
    ...
    return (
        <MainPage loading={loading}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 10 }}>
                <Space>
                    <div className="txt_title">អំពីមន្ទីរពិសោធន៍</div>
            ...
            <Modal
                title={isEditMode ? "កែប្រែ | អំពីមន្ទីរពិសោធន៍" : "បន្ថែម | អំពីមន្ទីរពិសោធន៍"}
                open={open}
                maskClosable={false}
