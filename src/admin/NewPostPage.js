        ...
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                multiFiles.forEach((file, index) => {
                    form.append(`multiImages[${index}]`, file);
                });

                const method = id ? "put" : "post";
                const url = id ? "tbpost/update" : "tbpost/create";
                    ...
                    <Row gutter={16}>
                        <Col xs={24} sm={24}>
                            <Form.Item
                                label="user"
                                name="userId"
                                rules={[{ required: true, message: "Please select a userId!" }]}
                            >
                                <Select>
                                    {category.map((item) => (
                                        <Select.Option key={item.id} value={item.id}>{item.userId}</Select.Option>
                                    ))}
                                </Select>
                            </Form.Item>
                        </Col>
